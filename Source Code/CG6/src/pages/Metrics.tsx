...
const defaultPredictions: HourlyPrediction[] = Array.from({ length: 24 }, (_, hour) => ({
  hour,
  temperature: 22 + Math.sin((hour / 24) * Math.PI * 2) * 5 + (Math.random() - 0.5) * 2,
  humidity: 60 + Math.sin((hour / 24) * Math.PI * 2) * 10 + (Math.random() - 0.5) * 5,
  co2: 700 + (hour > 8 && hour < 18 ? 150 : -50) + (Math.random() - 0.5) * 50,
  soil_moisture: 55 - hour * 0.3 + (Math.random() - 0.5) * 2,
}));
...
const metricRanges = [
  {
    ...
    warningMin: 10,
    warningMax: 35,
    description: 'Optimal temperature range for most greenhouse crops. Higher temperatures can cause heat stress, while lower temperatures slow growth.',
    zones: [
  ...
  },
  {
    ...
    warningMin: 35,
    warningMax: 85,
    description: 'Relative humidity affects transpiration and disease pressure. High humidity promotes fungal growth, while low humidity causes plant stress.',
    zones: [
  ...
  },
  {
    ...
    warningMin: 400,
    warningMax: 2000,
    description: 'CO₂ enrichment enhances photosynthesis and growth. Levels above 2000 ppm may indicate poor ventilation.',
    zones: [
  ...
  },
  {
    ...
    warningMin: 25,
    warningMax: 75,
    description: 'Soil moisture directly affects water uptake and nutrient availability. Waterlogged conditions cause root rot, while dry soil leads to wilting.',
    zones: [
...
const Metrics = () => {
  const [predictions, setPredictions] = useState<HourlyPrediction[]>(defaultPredictions);
  ...
  useEffect(() => {
    ...
    if (storedResult) {
      ...
      if (result.metrics) {
        setCurrentValues({
          temperature: result.metrics.temperature.avg,
          humidity: result.metrics.humidity.avg,
          co2: result.metrics.co2.avg,
          soil_moisture: result.metrics.soil_moisture.avg,
        });
      }
    }
  }, []);
